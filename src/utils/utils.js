...
/** form 表单中的 table render 相关函数方法,用法: render: moneyRender(record) */
/**
 * 金额的单元格，默认保留2位小数，否则需传入
 * @param fixNO
 * @returns {function(*=): *}
 */

export const moneyRender = fixNO =>
  // table的render参数
  (text = '') => <span>{formatMoney(text, fixNO)}</span>
 ...
 * @returns {function(*=): *}
 */
export const dateRender = (format = 'YYYY-MM-DD') =>
  // table的render参数
  (text = '') => <span>{formatDate(text, format) || '无'}</span>

/** 纯数值返回空状态下的展示
 * @returns {function(*=): *}
 */
export const pureValueRender = () => text => <span>{ text || '—'}</span>
